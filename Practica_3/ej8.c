...
void gestion(int numero_de_senhal);  // Función de gestión de SIGALRM
...
int main(){
    ...
    // sistema cuando son interrumpidas por un gestor de señal
    ...
    exit(EXIT_SUCCESS);     // Nunca se llegará a este exit()
}
...
void gestion(int numero_de_senhal){
    // No hace falta comprobar que la señal sea SIGALRM, porque es la única
    // que capturamos
