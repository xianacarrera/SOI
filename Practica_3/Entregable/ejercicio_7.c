...
#define ROJO "\e[1;31m"        // Color del texto del proceso padre
#define VERDE "\e[1;32m"       // Color para el proceso hijo
#define MAGENTA "\e[1;35m"     // Color para la gestión de señales
#define CYAN "\e[1;36m"        // Color para el proceso nieto
#define RESET "\e[0m"          // Color de errores (predeterminado)


// Función gestora de SIGUSR1
static void gestion_sigusr1(int numero_de_senhal);
// Función gestora de SIGUSR2
static void gestion_sigusr2(int numero_de_senhal);

// Función en la que trabaja H1
void ejecutar_h1(pid_t p, pid_t n1);
// Función en la que trabaja N1
void ejecutar_n1(pid_t p);
...
int main(){
    ...
    // Imprimimos también un mensaje con el PID de P para que después el
    // usuario pueda corroborar que es él el que está recibiendo las señales
    p = getpid();
    printf("\t%sMi PID es %d%s\n", ROJO, p, RESET);
    ...
    if (!(div_h1 = crear_proceso())){
        // H1 crea N1 y este salta a su repectiva función, llevando como
        // argumento el PID del proceso padre para poder enviarle una señal
    ...
    if (!WIFEXITED(status)){
        imprimir_mensaje_y_hora("Soy P.\n\tNo se pudo recuperar el PID de "
                "N1.", 1, RESET);
        exit(EXIT_FAILURE);
    }
...
void ejecutar_h1(pid_t p, pid_t n1){
    ...
    // H1 también se identifica mostrando su PID
