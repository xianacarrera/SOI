 ...
 */
void * ejecutar_hilo(void * p_num_hilo){
    int k;      // Contadores de la iteración actual de cada hilo
    int temp;   // Variables temporales para contar 100 iteraciones
    int clave = *((int *) p_num_hilo);    // Clave del hilo
    ...
    while (k < MAX_K){
        ...
        for (; k < temp; k++){
            ...
            pi_parcial[clave] += calcular_iteracion_pi(k);
